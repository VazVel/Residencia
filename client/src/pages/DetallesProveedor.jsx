...
const DetallesProveedor = () => {
  ...
  useEffect(() => {
    ...
    const fetchProveedor = async () => {
      try {
        // Endpoint para obtener detalle de un solo proveedor por body
        const res = await axios.post('/api/proveedores/detalle', { idproveedor: idGuardado });
        const d = res.data;
        
        // Limpieza de nulls para evitar errores en inputs controlados
        setFormData({
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      // Enviamos el objeto completo (incluye idproveedor) al endpoint PUT seguro
